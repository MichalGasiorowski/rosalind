# -*- coding: utf-8 -*-
"""
Created on Wed Apr 23 11:27:46 2014
...
its left, including itself (e.g., the positions of all occurrences of 'U' 
...
with open("rosalind_subs.txt", 'r') as f:
    data = f.readlines()
    s = data[0].strip()
    pattern = r'(?=(%s))' % data[1].strip()
    matches = re.finditer(pattern, s)    
    fo.write(" ".join( [str(match.span()[0] + 1) for match in matches ] )  )      
